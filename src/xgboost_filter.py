...
try:
    ...
    print("[INFO] XGBoost not available, using heuristic filter")
    ...
    def __init__(self, model_path: str = None):
        ...
        if XGBOOST_AVAILABLE and model_path and os.path.exists(model_path):
            try:
                ...
                print(f"[INFO] Loaded XGBoost filter model from {model_path}")
...
def train_filter_model(training_data: List[Tuple[Dict, int]], 
    ...
    if not XGBOOST_AVAILABLE:
        print("[ERROR] XGBoost not available for training")
        return False
    ...
    print(f"[INFO] Saved XGBoost filter model to {output_path}")
